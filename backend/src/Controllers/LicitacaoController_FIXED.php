...
/**
 * Controller: LicitacaoController (VERSÃO CORRIGIDA)
 *
 * Gerencia endpoints de licitações
 * - listar: Listagem simples (sem limite)
 * - buscar: Busca com filtros (sem limite) - CORRIGIDO PARA USAR FULLTEXT
...
class LicitacaoController
{
     ...
     *
     * ✅ CORRIGIDO: Usa FULLTEXT SEARCH para palavra-chave (muito mais rápido)
     */
    public function buscar(): array
    {
        try {
            ...
            // ✅ CORREÇÃO PRINCIPAL: Usar FULLTEXT SEARCH para palavra-chave
            if ($palavraChave) {
                // Limpar a palavra-chave
                $palavraChaveLimpa = trim($palavraChave);

                // Se tem mais de 3 caracteres, usar FULLTEXT SEARCH (rápido)
                if (strlen($palavraChaveLimpa) >= 3) {
                    // Preparar termo para FULLTEXT (adicionar wildcard)
                    $termoFulltext = $palavraChaveLimpa;

                    // Se não tem operadores booleanos, adicionar wildcard
                    if (!preg_match('/[+\-"*]/', $termoFulltext)) {
                        // Dividir em palavras e adicionar wildcard em cada
                        $palavras = explode(' ', $termoFulltext);
                        $palavras = array_filter($palavras); // remover vazios
                    ...
                    // Para palavras curtas (< 3 chars), usar LIKE tradicional
            ...
            return [
                'success' => false,
                'error' => 'ERRO_SERVIDOR',
                'message' => 'Erro ao buscar licitações: ' . $e->getMessage()
            ];
        }
    }
