/**
 * Inteligência de Preços V2 - Solução Híbrida
 * Backend: Dados do governo
 * Frontend: Busca no Mercado Livre + Cálculo de oportunidades
 */
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // NOVO: Não precisa mais verificar OAuth (não é necessário!)
...
// NOVO: Buscar oportunidades (solução híbrida)
async function buscarOportunidades(e) {
    ...
    try {
        // Passo 1: Buscar dados do governo (Backend)
        ...
        if (!responseGov.success || !responseGov.itens) {
            container.innerHTML = `
                <div class="alert alert-warning">
                    <i class="fas fa-exclamation-triangle"></i>
                    ${responseGov.error || 'Erro ao buscar dados do governo'}
                </div>
            `;
        ...
        const itensGoverno = responseGov.itens;
        ...
        // Passo 2: Buscar no Mercado Livre (Frontend - direto do navegador!)
        container.innerHTML = `
            <div class="loading-container">
                <div class="spinner"></div>
        ...
        const produtosML = await buscarMercadoLivre(termo);

        // Passo 3: Calcular oportunidades (Frontend)
        ...
        if (oportunidades.length === 0) {
            container.innerHTML = `
                <div class="alert alert-info">
                    ...
                    Encontramos <strong>${itensGoverno.length} licitações</strong> do governo,
                    mas não foi possível calcular oportunidades com os preços do Mercado Livre.
                </div>
                <div class="empty-state">
                    <i class="fas fa-chart-line"></i>
                    <h3>Nenhuma oportunidade de margem positiva</h3>
                    <p>Os preços do mercado estão acima dos preços do governo para "${termo}"</p>
                </div>
            `;
            return;
        }
...
// NOVO: Buscar produtos no Mercado Livre (direto do navegador!)
async function buscarMercadoLivre(termo, limit = 10) {
    try {
        ...
        if (!response.ok) {
            console.error('Erro ML:', response.status);
            return [];
        }
        ...
        })).filter(p => p.preco > 0); // Apenas produtos com preço
...
// NOVO: Calcular oportunidades (matching + margem)
function calcularOportunidades(itensGoverno, produtosML) {
    ...
    itensGoverno.forEach(itemGov => {
        ...
        if (melhorProduto) {
            ...
            // Apenas incluir se margem for positiva
            if (margem > 0) {
                oportunidades.push({
                    governo: {
                        item_id: itemGov.id,
...
// NOVO: Encontrar melhor match por similaridade
function encontrarMelhorMatch(itemGov, produtosML) {
    ...
    // Retornar se similaridade razoável (> 30%) ou primeiro produto
...
// Renderizar oportunidades (mesmo código anterior)
        ...
        html += `
            <div class="oportunidade-card">
                ...
                <div class="acoes-footer">
                    <div>
                        ${merc.permalink ? `<a href="${merc.permalink}" target="_blank" class="btn-acao btn-comprar"><i class="fas fa-shopping-cart"></i> Ver no ML</a>` : ''}
                    </div>
