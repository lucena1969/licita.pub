...
const LicitacoesModule = {
    ...
    async carregarLicitacoes() {
        ...
        try {
            ...
            // Fazer requisição
            const response = await api.request(`/licitacoes/buscar.php?${params.toString()}`);

            // api.request encapsula: { success, status, data: <payload da API> }
            // payload da API: { success: true, data: [licitacoes], paginacao: {...} }
            if (response.success && response.data) {
                const apiData = response.data;

                if (apiData.success && apiData.data) {
                    // apiData.data é o array de licitações
                    this.state.licitacoes = apiData.data || [];
                    this.state.paginacao.totalItens = apiData.paginacao?.total || 0;
                    this.state.paginacao.totalPaginas = apiData.paginacao?.total_paginas || 1;
        ...
        const itemsHTML = this.state.licitacoes.map(lic => `
            <div class="item">
                ...
                <div class="item-actions">
                    <button class="btn btn-primary btn-sm" onclick="LicitacoesModule.verDetalhes('${lic.pncp_id}')">
    ...
    verDetalhes(id) {
        if (!id) {
            console.error('ID da licitação não fornecido');
            alert('Erro: ID da licitação não encontrado');
            return;
        }

        console.log('Abrindo detalhes da licitação:', id);

        // Codificar o ID para evitar problemas com caracteres especiais
        const encodedId = encodeURIComponent(id);
        const url = `/frontend/detalhes.html?id=${encodedId}`;
