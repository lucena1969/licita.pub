...
// Período (padrão: próximos 60 dias para licitações abertas)
if (isset($opcoes['ultimos-dias'])) {
    // Modo legado: buscar contratos antigos
    $diasAtras = (int)$opcoes['ultimos-dias'];
    $filtros['dataInicial'] = date('Ymd', strtotime("-{$diasAtras} days"));
    $filtros['dataFinal'] = date('Ymd');
} else {
    // Modo novo: buscar licitações abertas (futuras)
    $filtros['dataFinal'] = date('Ymd', strtotime('+60 days'));
}
...
// Página inicial (para importação em lotes)
if (!empty($opcoes['pagina'])) {
    $filtros['pagina'] = (int)$opcoes['pagina'];
}

// Máximo de páginas (para importação em lotes)
if (!empty($opcoes['max-paginas'])) {
    $filtros['maxPaginas'] = (int)$opcoes['max-paginas'];
}
